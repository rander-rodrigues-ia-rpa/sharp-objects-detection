...
# Se você já fez download do dataset manualmente, defina o caminho aqui
...
# Defina a configuração para o treinamento
def criar_arquivo_config(nome_arquivo="data.yaml"):
    ...
    if not os.path.exists(f"{caminho_dataset}/{nome_arquivo}"):
        ...
        with open(f"{caminho_dataset}/{nome_arquivo}", 'w') as f:
            ...
            f.write("names: ['objeto_cortante']\n")  # Ajuste os nomes das classes conforme necessário
    else:
...
def treinar_modelo(config_path, epochs=100, imgsz=640, batch=16):
    ...
    # Nota: Como YOLOv12 ainda não está disponível na ultralytics, usaremos YOLOv8
    # Quando YOLOv12 estiver disponível, basta substituir 'yolov8n.pt' por 'yolov12n.pt'
